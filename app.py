...
def analyze_with_local_ai(reasoning_text):
    try:
        ...
        def rule_based_analysis(text):
            ...
            if max([emotional_score, logical_score, intellectual_score]) - min(
                    [emotional_score, logical_score, intellectual_score]) < 15:
            ...
            if any(w in lower for w in
                   ['obviously', 'clearly', 'definitely will', 'certain', 'sure thing', 'no doubt']):
        ...
        def index():
            ...
            c.execute(
                'select d.*, r.actual_outcome, r.revised_perspective, r.lessons_learned, r.would_decide_same, r.reflection_analysis from decisions d left join reflections r on d.id = r.decision_id where d.user_id = ? order by d.date_created desc',
                (user_id,))
            rows = c.fetchall()
            conn.close()
            decisions = []
        ...
        def add_reflection(decision_id):
            ...
            try:
                ...
                c.execute(
                    ...
                    (decision_id, user_id, data['actual_outcome'], data['revised_perspective'],
        ...
        def get_advice():
            ...
            c.execute(
                'select d.title, d.context, d.decision, d.full_reasoning, d.initial_analysis, d.has_reflection, r.actual_outcome, r.revised_perspective, r.would_decide_same from decisions d left join reflections r on d.id = r.decision_id where d.user_id = ? order by d.date_created desc limit 10',
                (user_id,))
            rows = c.fetchall()
            conn.close()
